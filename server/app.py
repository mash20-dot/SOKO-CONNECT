...
def login():
        ...
        # Step 2: If user exists and password matches
...
def product():
        ...
        product_uses = data.get('pro_uses')
        product_images = data.get('pro_images')
       ...
       #saves products/items into the database
        ...
        if (product_name and product_price and product_uses and product_images):
             return jsonify({'message': 'Item updated successfully '}), 201
        
        if product in (product_name and product_price and product_uses and product_images):
             return jsonify({'message': 'Item already in stock'})
        
        if not (product_name and product_price and product_uses and product_images):
...
def business():
    ...
    if not (business_name and business_email and phone):
...
def getbusiness():
     ...
     data = request.get_json()
     business_name = data.get('bsiness_name')
     business_email = data.get('business_email')
     phone = data.get('phone')

     user = Business_info.query.filter_by(email=business_email).first()
     
     
     #conditional statements
     if (business_name and business_email and phone):
          return jsonify({'message': 'Information retrieved successfully'})
     if not (business_name and business_email and phone):
          return jsonify({'message': 'Error, information could not be retrieved'})
