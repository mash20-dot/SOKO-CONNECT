...
def history():
        ...
        new_user = History(buyer_name=buyer_name, buyer_product=buyer_product, date=date, email=current_email)
        db.session.add(new_user)
        db.session.commit()
        
        return jsonify({"message": "saved successfully", "logged_in_as":current_email}), 200
